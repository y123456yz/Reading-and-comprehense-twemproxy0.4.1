    ...
    for (p = r->pos; p < b->last; p++) {
        ...
        case SW_REQ_TYPE:
            if (ch == ' ' || ch == CR) {
                ...
                case 3:
                    if (str4cmp(m, 'g', 'e', 't', ' ')) {  //get kye1 key2  get可以一次获取多个key
                ...
                case 4:
                    if (str4cmp(m, 'g', 'e', 't', 's')) {//gets kye1 key2  gets可以一次获取多个key
                            ...
                            uint32_t key_step)
{
    ...
    /* 把分布在同一个memcached服务器上面的get请求拼接到一起，例如get key1 key2 key3 key4 key5中的key2 key3分布在同一个服务器，
        则组成字符串"key2 key3"并由对应的后端服务器指向*/
    ...
    //把get添加到 "key2 key3"字符串前面组成字符串 "get key2 key3"
    for (i = 0; i < ncontinuum; i++) {     /* prepend mget header, and forward it */
        ...
        TAILQ_INSERT_TAIL(frag_msgq, sub_msg, m_tqe); //把所有的字符串组好后添加到队列frag_msgq
        r->nfrag++;
    }
 ...
 */ //get key1 key2 key3 key4 ....，后端应答回来的多个请求，在这里进行合并
void
