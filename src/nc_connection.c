...
//分配连接conn加1，释放连接减1，见conn_put _conn_get   表示当前客户端连接数
...
conn_get(void *owner, bool client, bool redis)
{
    ...
    if (conn->client) { //对端是客户端，也就是本twemproxy为服务端  proxy_accept接收到新的连接，会accept返回新的套接字，该新套接字就相当于一个client
        /*
        ...
        conn->send = msg_send;
        conn->send_next = rsp_send_next; //发往客户端用rsp_send_next 发往后端服务器用req_send_next    msg_send或者msg_send_chain中执行
         ...
         * request upstream.
         */
         //对待不是客户端，也就是对待是服务器端，说明对待是redis服务器或者memcached服务器
        ...
        //向后端服务器发送请求
        conn->send = msg_send;
        conn->send_next = req_send_next; //发往客户端用rsp_send_next 发往后端服务器用req_send_next    msg_send或者msg_send_chain中执行
...
uint32_t
conn_ncurr_conn_zero(void)
{
    return ncurr_conn;
}

uint64_t
conn_ntotal_conn_zero(void)
{
    return ntotal_conn;
}
...
conn_authenticated(struct conn *conn)
{
    ...
    if (!pool->require_auth) { //不需要认证
        return true;
    }

    if (!conn->authenticated) { //配置需要认证并且已经认证成功
        return false;
    }
