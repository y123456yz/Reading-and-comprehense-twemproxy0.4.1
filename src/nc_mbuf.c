...
/*
*一个mbuf空间由data+struct(mbuf)组成，注意struct(mbuf)在整个mbuf_chunk_size最末尾处, 参考_mbuf_get
     *   <------------- mbuf_chunk_size ------------->
     *   +-------------------------------------------+
     *   |       mbuf data          |  mbuf header   |  mbuf data部分为可用空间，mbuf header用来记录struct mbuf
     *   |     (mbuf_offset)        | (struct mbuf)  |
     *   +-------------------------------------------+
*/
//赋值见mbuf_init
static size_t mbuf_chunk_size; /* mbuf chunk size - header + data (const) */
static size_t mbuf_offset;     /* mbuf offset in chunk (const) */ //也就是上面的mbuf data部分
...
_mbuf_get(void)
{
     ...
     * put operations
     *一个mbuf空间由data+struct(mbuf)组成，注意struct(mbuf)在整个mbuf_chunk_size最末尾处
     *   <------------- mbuf_chunk_size ------------->
     *   +-------------------------------------------+
     ...
     *
     */
    mbuf = (struct mbuf *)(buf + mbuf_offset); //一个mbuf空间由data+struct(mbuf)组成，注意struct(mbuf)在整个mbuf_chunk_size最末尾处
...
mbuf_get(void)
{
    ...
    buf = (uint8_t *)mbuf - mbuf_offset; //指向mbuf data头部处
    mbuf->start = buf;
...
//只有在进程退出的时候才会释放，见core_stop->mbuf_deinit
static void
...
//回收mbuf，把该mbuf放入可重复利用队列，以便下次有新连接到来的时候直接使用，注意这里不是真正释放空间
void
