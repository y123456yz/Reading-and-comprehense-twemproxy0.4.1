...
core_calc_connections(struct context *ctx)
{
    ...
    status = getrlimit(RLIMIT_NOFILE, &limit); //进程描述符最大数
    if (status < 0) {
...
//解析配置文件，并未解析出的bind或者servers:列表创建相应的套接字
...
core_ctx_create(struct instance *nci)
{
    ...
    ctx->cf = conf_create(nci->conf_filename); //配置文件的解析也在该函数里面
    if (ctx->cf == NULL) {
        nc_free(ctx);
        return NULL;
    }
     ...
     */
    status = core_calc_connections(ctx); //计算客户端最多使用的并发连接fd
    if (status != NC_OK) {
    ...
    /* create stats per server pool */ //stats状态信息初始化
    ...
    //epoll事件回调赋值在event_base_create，执行在event_wait，事件回调函数为core_core
    ctx->evb = event_base_create(EVENT_SIZE, &core_core); //epoll初始化
    if (ctx->evb == NULL) {
    ...
    //开始创建conn连接，来和后端connect，根据的是后端的servers:列表地址来建立连接
    ...
    status = proxy_init(ctx); //为alpha等大server的bind创建套接字并接收客户端连接
    if (status != NC_OK) {
        server_pool_disconnect(ctx);
        event_base_destroy(ctx->evb);
        stats_destroy(ctx->stats);
...
core_start(struct instance *nci)
{
    ...
    if (ctx != NULL) {
        nci->ctx = ctx; //正常情况走到这里
        return ctx;
    }

    //异常情况对msg conn mbuf释放空间
    conn_deinit();
    msg_deinit();
    mbuf_deinit();
...
core_send(struct context *ctx, struct conn *conn)
{
    ...
    status = conn->send(ctx, conn); //msg_send
    if (status != NC_OK) {
...
core_close(struct context *ctx, struct conn *conn)
{
    ...
    //nc_core.c:238 close c 7 '127.0.0.1:54136' on event 00FF eof 0 done 0 rb 5 sb 0: Invalid argument
...
core_timeout(struct context *ctx)
{
    for (;;) {
        ...
        msg = msg_tmo_min();
        if (msg == NULL) {
            ctx->timeout = ctx->max_timeout; //默认下次以max_timeout
            return;
        }

        /* skip over req that are in-error or done */

        if (msg->error || msg->done) { //已经处理了的msg  这里的定时器添加在req_forward->req_server_enqueue_imsgq
        //发送数据到后端的时候添加定时器，然后等待后端应答并添加定时器，这里表示在规定时间内收到了后端应答
            msg_tmo_delete(msg); 
            continue;
        }
        ...
        //和后端服务器已超时，则从红黑树中删除该msg信息同时关闭连接
...
{ //core_core在core_loop中执行
    ...
    //是proxy接收到客户端连接，还是客户端conn产生epoll事件，还是后端服务器产生epoll事件
