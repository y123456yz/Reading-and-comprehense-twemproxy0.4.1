...
proxy_each_init(void *elem, void *data)
{
    ...
    log_debug(LOG_INFO, "p %d listening on '%.*s' in %s pool %"PRIu32" '%.*s'"
...
{//为alpha等大server的bind创建套接字并接收客户端连接
    rstatus_t status;
...
proxy_recv(struct context *ctx, struct conn *conn)
{
    ...
    } while (conn->recv_ready); //proxy_accept中把所有客户端连接都接收完毕后会置recv_ready为0
